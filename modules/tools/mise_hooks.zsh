# ==============================================================================
# Mise Hooks avec Post-Installation Work
# ==============================================================================
# Intercepte les commandes mise install pour appliquer automatiquement
# les configurations Work (certificats Java, settings Maven)
...
# Verifie si un tool@version est deja configure Work
# Usage: _mise_is_configured java zulu-21
_mise_is_configured() {
    local tool=$1 version=$2
    [[ -f "$_MISE_INSTALLS_DIR/$tool/$version/.work_configured" ]]
}
...
_mise_mark_configured() {
    local tool=$1 version=$2
    local marker="$_MISE_INSTALLS_DIR/$tool/$version/.work_configured"
...
mise-configure() {
    ...
    if [[ -z "$tool" ]]; then
        ...
        echo "  java    - Importe les certificats Work"
        echo "  maven   - Deploie settings.xml dans ~/.m2/"
        echo "  status  - Affiche l'etat de la config Work"
        echo ""
        echo "Exemples:"
...
_mise_prompt_configure() {
    ...
    echo -n "Appliquer la config Work ? [y/N] "
    local reply
    read -r reply
    if [[ "$reply" =~ ^[yY]$ ]]; then
        local tool version
...
_mise_chpwd_hook() {
    ...
    typeset -f work_is_context > /dev/null 2>&1 && work_is_context || return
    [[ -f ".mise.toml" ]] || return
